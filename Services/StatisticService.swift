...
class StatisticService: StatisticServiceProtocol {
    ...
    func showStatistic(currentGameResult: Int, currentGameRoundsCount: Int) -> String {
        return "Ваш результат: \(currentGameResult)/\(currentGameRoundsCount)\nКоличество сыгранных квизов: \(self.gamesCount)\nРекорд: \(self.bestGame.correctRounds)/\(self.bestGame.rounds) (\(self.bestGame.date.dateTimeString))\nСредняя точность: \(String(format: "%.2f", countAverageWins(games: self.roundsCount, wins: self.winsRoundCount)))%"
    }
}
